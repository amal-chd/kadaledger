...
export default function RegisterPage() {
    ...
    const plan = searchParams.get('plan'); // e.g., 'trial'

    const handleSendOtp = async (e: React.FormEvent) => {
        e.preventDefault();
        setLoading(true);
        try {
            await authApi.login(phone); // Assuming login handles upsert/registration too
            setStep('OTP');
        } catch (err) {
            ...
            <div className="max-w-md w-full glass-card p-10 rounded-[2.5rem] relative z-10 border border-white/10 shadow-2xl backdrop-blur-xl">
                ...
                {step === 'PHONE' ? (
                    ...
                    <form onSubmit={handleVerify} className="space-y-6 animate-fade-in-up">
                        <div>
                            <label className="block text-sm font-medium text-blue-200/60 mb-2 pl-1">Verification Code</label>
                            <input
                                type="text"
                                value={otp}
                ...
                <div className="mt-8 text-center text-sm text-blue-200/60">
                    Already have an account? <Link href="/login" className="text-blue-400 hover:text-white hover:underline transition-colors font-medium">Login</Link>
                </div>
            </div>
        </div>
    );
