...
function TemplatesView() {
    // Mock Data
    const templates = [
        { id: 1, name: 'otp_verification', language: 'en', status: 'APPROVED' },
        { id: 2, name: 'payment_reminder', language: 'en', status: 'APPROVED' },
        { id: 3, name: 'payment_reminder_malayalam', language: 'ml', status: 'PENDING' },
    ];
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {templates.map(t => (
                    <div key={t.id} className="bg-slate-800 p-6 rounded-2xl border border-slate-700 hover:border-slate-600 transition-all group">
                        ...
                        <p className="text-slate-400 text-sm">Language: {t.language === 'en' ? 'English' : 'Malayalam'}</p>
                    </div>
                ))}
            </div>
        </div>
